  ...
  return (
    <div
      className={
        ...
        (selectedProject === "0" && projectData === null)
    ...
    >
      <div
        className={`flex flex-row w-full justify-between items-center ${height}`}
      >
        <div className="flex items-center">
          ...
          <div className={styles.home_savedNoteDetails}>
            <h3 className="capitalize">
              {projectData?.name ?? "Default project"}
            </h3>
          </div>
        </div>
        <div
...
export const SavedNotes = () => {
  ...
  const notesFilteredByUser = notes?.filter(
    (note) => note.user_id === selectedUserId
  );
  ...
  useEffect(() => {
    if (
      selectedProject === "" &&
      uniqueProjectStreams &&
      uniqueProjectStreams.length > 0
    )
